...
- (QCloudSignature *)signatureForData:(NSMutableURLRequest *)urlrequest {
    if (!self.credential.secretID.length) {
        @throw [NSException exceptionWithName:@"com.tencent.qcloud.error" reason:@"请检查 secretID 是否为空" userInfo:nil];
    }
    if (!self.credential.secretKey.length) {
        @throw [NSException exceptionWithName:@"com.tencent.qcloud.error" reason:@"请检查 secretKey 是否为空" userInfo:nil];
    }

    if ([self.credential.secretID hasPrefix:@" "] || [self.credential.secretID hasPrefix:@" "] ) {
        @throw [NSException exceptionWithName:@"com.tencent.qcloud.error" reason:@"请检查 secretID 是否合法" userInfo:nil];
    }
